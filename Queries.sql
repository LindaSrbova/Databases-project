...
--B. ADVANCED QUERIES
-- Create a view that lists beneficiaries from highest number of fully fitting applications for it (agreed on skills, interest, validity, application wasnt modified after deadline) that have passed the registration date
--(the script assuming there has been the ‘normalized_title’ created in request, in Query no.6, which defines the interest area in the same format as interest_assignment)

CREATE VIEW Beneficiaries_With_most_match AS
SELECT b."name", count(distinct va.id) as matching_applications -- listing beneficiaries from the highest to lowest number of failed applications
FROM beneficiary b 
JOIN request r ON b.id = r.beneficiary_id -- joining beneficiaries and requests by matching beneficiary id
JOIN volunteer_application va ON r.id = va.request_id -- joining volunteer applications by matching request ids
join request_skill rs on rs.request_id = r.id -- joining requested skills by matching request ids
join skill_assignment sa on sa.volunteer_id = va.volunteer_id -- joining volunteer skills by matching volunteer id
...
and va.is_valid = TRUE-- selecting rows for the registration date has passed, the application wasnt modified after the deadline, the skills matched, the applicant's interests matched, the application was valid, 
GROUP BY b.name
